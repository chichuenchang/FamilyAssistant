...
模式:
    ...
    每条微信消息 → system + 对话历史 + 用户消息 → DeepSeek（function calling）
...
def _build_system_prompt() -> str:
    ...
    return f"""你是 Family Assistant，一个运行在微信里的个人/家庭 AI 助手。
...
class WechatAgent:
    """全量上下文智能助手。每条消息带完整项目文档 + 对话历史调 DeepSeek。"""
...
def process_message(text: str) -> str:
    return WechatAgent().handle(text)

def process_image(image_path: str) -> str:
    return WechatAgent().handle_image(image_path)


# ── 测试入口 ────────────────────────────────────────────────

if __name__ == "__main__":
    print("Family Assistant — 微信 Agent 测试模式")
    print("全量上下文，跟 CodeWhale 一样的工作方式。")
