...
- **工作表**：长期、可原地更新的结构化记录 —— kv 事实清单（房贷利率/保单号）或 table 流水（血压/成绩打卡）；置顶后每次对话自动带全表
- **数据可视化**：「把我2025年的成绩画个图」→ Agent 取出对应数字渲染折线/柱状/饼图，图片直接发回微信/Telegram（离线渲染，数据不出本机）
...
setx ZHIPU_API_KEY "xxx"              # 可选：glm-5.3-flash（/model glm）；也是 DeepSeek 静默 30s 后的自动顶替
...
setx ZHIPU_API_KEY "xxx"              # 可选：GLM 备用模型 + DeepSeek 宕机自动顶替
